...
inline bool Layer::isDisconnectedStrip(int strip)
{
	if(!m_isSM1)
	{
		...
		if(m_layerIndex==0 && (strip==1214 || (strip>=1219 && strip<=1231 ) || strip==1234 || strip==1235 || strip==1248 || strip==1256 || strip==1264 || strip==1271 || strip==1272 || strip==1273 || strip==1275 || strip==1277 || strip==1848 || strip==1909 )) return true;
		else if(m_layerIndex==1 && (strip==1833 || strip==1999)) return true;
